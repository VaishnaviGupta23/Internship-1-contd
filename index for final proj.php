...
if (isset($uniqueUrls) && is_array($uniqueUrls) && count($uniqueUrls) > 0) {
    $totalUniqueUrls = count($uniqueUrls);
    echo '<p>Total unique URLs found: ' . $totalUniqueUrls . '</p>';
    ...
    foreach ($uniqueUrls as $entry) {
        ...
        $maxLength = 50; 
        $displayUrl = strlen($url) > $maxLength ? substr($url, 0, $maxLength) . '...' : $url;

        echo '<tr><td>' . $id . '</td><td><a href="' . htmlspecialchars($url) . '">' . htmlspecialchars($displayUrl) . '</a></td></tr>';
    }
    echo '</tbody>';
    echo '</table>';
} 
